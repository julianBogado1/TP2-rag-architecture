...
OVERFETCH_MULTIPLIER = 6                       # Alternative: fixed 500 — simpler but ignores per-request top_k.
...
class VectorRetrievalService:
    ...
    def _passes_filters(meta: dict, chars: dict, filters: MetadataFilters) -> bool:
        ...
        if filters.genres_not_in and (set(genres) & set(filters.genres_not_in)):
            return False
        if filters.artist_in and not any(a.lower() in artist for a in filters.artist_in):
            return False
        if filters.artist_not_in and any(a.lower() in artist for a in filters.artist_not_in):
            return False
